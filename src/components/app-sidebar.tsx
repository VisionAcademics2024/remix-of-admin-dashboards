...
/**
 * The ornament - the pointer navigation.
 *
 ...
 *
 * That gesture needs a pointer, so the rail is desktop-only. A phone gets
 * MobileNav instead - a button in the header that opens the same sections as a
 * list you tap. Both read from nav-items.ts, so neither can fall behind.
 *
...
export function AppSidebar({
  ...
  return (
    <nav
      ...
      className={cn(
        "ornament animate-ornament-in fixed left-4 top-1/2 z-40 hidden max-h-[calc(100vh-1.5rem)] -translate-y-1/2 flex-col gap-1 overflow-y-auto overflow-x-hidden rounded-[2rem] p-2.5 transition-[width] duration-[320ms] lg:flex",
